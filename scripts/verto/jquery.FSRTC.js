...
(function($) {
    ...
    function onStreamError(self, e) {
        if(__env.enableDebug) console.log('There has been a problem retrieving the streams - did you allow access? Check Device Resolution', e);
...
3
	...
	if (self.options.useVideo && self.options.localVideo) {
		...
		onsuccess: function(e) {self.options.localVideoStream = e; if(__env.enableDebug) console.log("local video ready");},
		onerror: function(e) {if(__env.enableDebug) console.error("local video error!");}
            });
	}
	...
	var gathering = false, done = false;
        ...
        if (options.iceServers) {
            ...
            if (tmp && !(typeof(tmp) == "object" && tmp.constructor === Array)) {
                if(__env.enableDebug) console.warn("iceServers must be an array, reverting to default ice servers");
                tmp = null;
            }
	...
	getUserMedia({
		...
		if(__env.enableDebug) console.info(width + "x" + height + " supported."); $.FSRTC.validRes.push([width, height]); checkRes(cam, func);},
	    onerror: function(e) {if(__env.enableDebug) console.error( width + "x" + height + " not supported."); checkRes(cam, func);}
        });
    }
	...
	getUserMedia({
		...
		if (check_video && check_audio) {
		    if(__env.enableDebug) console.error("error, retesting with audio params only");
