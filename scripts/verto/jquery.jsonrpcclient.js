...
(function($) {
   ...
   */
    ...
    // Construct the JSON-RPC 2.0 request.
        ...
        if (!success_cb) {
            success_cb = function(e){ if(__env.enableDebug) console.log("Success: ", e);};
        }

        if (!error_cb) {
            error_cb = function(e){if(__env.enableDebug) console.log("Error: ", e);};
        }
        ...
        $.ajax({
            ...
            error    : function(jqXHR, textStatus, errorThrown) {
                try {
                    ...
                    if ('console' in window) if(__env.enableDebug) console.log(response); // Ugly, but it should work.
	...
	var self = this;
        if (self.socketReady()) {
            self._ws_socket.onclose = function (w) {if(__env.enableDebug) console.log("Closing Socket");};
        ...
        if (!self.socketReady()) {
            ...
            if (self._ws_socket) {
                ...
                self._ws_socket.onclose = function (w) {
                    ...
                    if(__env.enableDebug) console.error("Websocket Lost " + self.ws_cnt + " sleep: " + self.ws_sleep + "msec");

                    self.to = setTimeout(function() {
                        if(__env.enableDebug) console.log("Attempting Reconnection....");
	...
	if (event.data[0] == "#" && event.data[1] == "S" && event.data[2] == "P") {
		...
		if(__env.enableDebug) console.info("Speed Test: Up: " + up_kps + " Down: " + down_kps);
                    ...
                    if (!self.authing && response.error.code == -32000 && self.options.login && self.options.passwd) {
                        ...
                        this.call("login", { login: self.options.login, passwd: self.options.passwd, loginParams: self.options.loginParams,
					     userVariables: self.options.userVariables},
                            ...
                            function(e) {
                                self.authing = false;
                                if(__env.enableDebug) console.log("logged in, resending request id: " + response.id);
                            ...
                            function(e) {
                                if(__env.enableDebug) console.log("error logging in, request id:", response.id);
                                delete self._ws_callbacks[response.id];
                                error_cb(response.error, this);
                                if (self.options.onWSLogin) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if ('error' in response) {
                if (response.id === null || !(response.id in handlers)) {
                    ...
                    if ('console' in window) if(__env.enableDebug) console.log(response);
                } else {
