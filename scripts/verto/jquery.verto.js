...
(function($) {
		...
		dialog = null;
            ...
            if (dialog) {
                ...
                case 'verto.info':
                    ...
                    break;
                default:
                    if(__env.enableDebug) console.debug("INVALID METHOD OR NON-EXISTANT CALL REFERENCE IGNORED", dialog, data.method);
                    break;
                }
            } else {
                ...
                case 'verto.invite':
                    ...
                    break;
                default:
                    if(__env.enableDebug) console.debug("INVALID METHOD OR NON-EXISTANT CALL REFERENCE IGNORED");
                    break;
                }
            }
		...
		break;
            case 'verto.event':
                ...
                if (!list && key && key === verto.sessid) {
                    ...
                    if(__env.enableDebug) console.error("UNSUBBED or invalid EVENT " + key + " IGNORED");
                } else {
                    for (var i in list) {
                        var sub = list[i];

                        if (!sub || !sub.ready) {
                            if(__env.enableDebug) console.error("invalid EVENT for " + key + " IGNORED");
                        } else if (sub.handler) {
            ...
            case "verto.info":
                ...
                //console.error(data);
                if(__env.enableDebug) console.debug("MESSAGE from: " + data.params.msg.from, data.params.msg.body);

                break;

            default:
                if(__env.enableDebug) console.error("INVALID METHOD OR NON-EXISTANT CALL REFERENCE IGNORED", data.method);
                break;
            }
        }
    };
    ...
    var hashArray = function() {
        ...
        vha.del = function(name) {
            ...
            if (hash[name]) {
                ...
                if(__env.enableDebug) console.error("can't del nonexistant key " + name);
            }
	...
	function genArray(obj) {
        ...
        la.onChange = function(obj, args) {
            ...
            if (dt && (args.action == "del" || args.action == "modify")) {
                ...
                if (index === undefined) {
                    if(__env.enableDebug) console.error("INVALID PACKET Missing INDEX\n", args);
                    return;
                }
            }
	...
	dialog.useSpeak = dialog.params.useSpeak;
        ...
        RTCcallbacks.onICESDP = function(rtc) {
            if(__env.enableDebug) console.log("RECV " + rtc.type + " SDP", rtc.mediaData.SDP);
	...
	if (typeof element.sinkId !== 'undefined') {
	    ...
	    if(__env.enableDebug) console.info("Dialog: " + dialog.callID + " Setting speaker:", element, devname);

	    element.setSinkId(sinkId)
		.then(function() {
		    if(__env.enableDebug) console.log("Dialog: " + dialog.callID + ' Success, audio output device attached: ' + sinkId);
		    if (callback) {
	    ...
	    if(__env.enableDebug) console.warn("Dialog: " + dialog.callID + ' Browser does not support output device selection.');
	    if (callback) {
		callback(false, null, arg);
	    }
	}
    ...
    $.verto.dialog.prototype.setState = function(state) {
        ...
        if (dialog.state == state || !checkStateChange(dialog.state, state)) {
            if(__env.enableDebug) console.error("Dialog " + dialog.callID + ": INVALID state change from " + dialog.state.name + " to " + state.name);
            dialog.hangup();
            return false;
        }

        if(__env.enableDebug) console.log("Dialog " + dialog.callID + ": state change from " + dialog.state.name + " to " + state.name);
    ...
    $.verto.dialog.prototype.handleAnswer = function(params) {
        ...
        if (dialog.state.val >= $.verto.enum.state.early.val) {
            ...
            if (dialog.gotEarly) {
                if(__env.enableDebug) console.log("Dialog " + dialog.callID + " Got answer while still establishing early media, delaying...");
            } else {
                if(__env.enableDebug) console.log("Dialog " + dialog.callID + " Answering Channel");
                ...
                if(__env.enableDebug) console.log("Dialog " + dialog.callID + "ANSWER SDP", params.sdp);
            }
        }
    ...
    $.verto.dialog.prototype.handleMedia = function(params) {
        ...
        dialog.rtc.answer(params.sdp, function() {
            if(__env.enableDebug) console.log("Dialog " + dialog.callID + "Establishing early media");
	...
	if ((!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) && MediaStreamTrack.getSources) {
		...
		.then(function(devices) {
			...
			if(__env.enableDebug) console.log(device.kind + ": " + device.label +
				    " id = " + device.deviceId);
		...
		.catch(function(err) {
		    if(__env.enableDebug) console.log(" Device Enumeration ERROR: " + err.name + ": " + err.message);
		    runtime(false);
		});
	}
